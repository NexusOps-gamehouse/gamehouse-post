...
-- (hbm2ddl.create_namespaces), duo_post 같은 일반 계정에는 DB CREATE 권한이 없다.
...
SELECT 'CREATE ROLE duo_post LOGIN'
 WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'duo_post') \gexec

-- 있든 없든 비밀번호를 지금 값으로 맞춘다.
ALTER ROLE duo_post WITH PASSWORD :'svc_pw';

-- AUTHORIZATION 이 핵심이다. 소유자가 되어야 ddl-auto: update 가 그 안에
-- 테이블을 만들 수 있다. GRANT 로 읽기·쓰기만 주면 부팅 중 테이블 생성에서 막힌다.
CREATE SCHEMA IF NOT EXISTS post_svc AUTHORIZATION duo_post;
